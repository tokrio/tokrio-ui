...
import { ApiKey, api, TokenPair, TokenPairsGroup, SetDexUsdtParam, TokenBalanceProps } from '../services/api';
...
const SettingDexUi: React.FC<SettingUiProps> = ({
  isOpen,
  tokenAccountId,
  usdtBalance,
  initUsdt,
  ...
  const [formData, setFormData] = useState<SetDexUsdtParam>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let param = {
        tokenAccountId: tokenAccountId,
        usdtAmount: new BigNumber(formData.usdtAmount).multipliedBy(10 ** usdtBalance.decimals).toFixed(0),
      }
      const response = await api.setDexUsdt(param);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div>{initUsdt} USDT</div>
            <label className="block text-sm mt-4 font-medium text-gray-300 mb-1">
              Adjust USDT Amount
            </label>
            <input
              type="number"
