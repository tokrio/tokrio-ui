...
import { CreateApiKeyRequest, TokenBalanceProps, WithdrwaRequest } from '../services/api';
...
export const WithdrawTradePop: React.FC<DepositProps> = ({ isOpen,usdtBalance, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if(new BigNumber(formData.amount).isGreaterThan(new BigNumber(usdtBalance.uiBalance))){
  ...
  return (
    <div className="fixed inset-0 bg-black/50 text-sm  backdrop-blur-sm z-50 flex items-center justify-center">
      ...
      >
        ...
        <div className='text-amber-400'>Balance:{usdtBalance.uiBalance}</div>
